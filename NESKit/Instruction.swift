...
extension CPU6502 {
    enum Opcode: String, Codable {
        ...
        static func size(for opcode: UInt8) throws -> Int {
            switch opcode {
            case 0xB8, 0xD8, 0x9A, 0xBA, 0x48, 0x68, 0x08, 0x28, 0xF8, 0x60, 0x18, 0x38, 0xEA, 0x40, 0x4A, 0x6A, 0x2A, 0x00, 0x78, 0xAA, 0x8A, 0xCA, 0xE8, 0xA8, 0x98, 0x88, 0xC8:
                return 1
            case 0x84, 0x94, 0xE9, 0xE5, 0xF5, 0xE1, 0xF1, 0xC0, 0xC4, 0xE0, 0xE4, 0xA0, 0xA4, 0xB4, 0x69, 0x65, 0x75, 0x61, 0x71, 0x49, 0x45, 0x55, 0x41, 0x51, 0x09, 0x05, 0x15, 0x01, 0x11, 0xC9, 0xC5, 0xD5, 0xC1, 0xD1, 0x29, 0x25, 0x35, 0x21, 0x31, 0x30, 0x50, 0x90, 0xB0, 0x86, 0x96, 0x70, 0x46, 0x56, 0xE6, 0xF6, 0x66, 0x76, 0x26, 0x36, 0x10, 0xF0, 0x24, 0x81, 0x91, 0x85, 0x95, 0xA1, 0xB1, 0xA9, 0xA5, 0xB5, 0xA2, 0xA6, 0xB6, 0xD0:
                return 2
            case 0x8C, 0xED, 0xFD, 0xF9, 0xEC, 0xCC, 0xAC, 0xBC, 0x6D, 0x7D, 0x79, 0x4D, 0x5D, 0x59, 0x0D, 0x1D, 0x19, 0xCD, 0xDD, 0xD9, 0x2D, 0x3D, 0x39, 0x8E, 0x4E, 0x5E, 0x6E, 0x7E, 0x2E, 0x3E, 0x2C, 0xAD, 0xBD, 0xB9, 0x20, 0x8D, 0x9D, 0x99, 0xAE, 0xBE, 0x4C, 0x6C, 0xEE, 0xFE:
                return 3
            default:
    ...
    struct Instruction {
        ...
        enum Operand {
            ...
            init(_ data: Data) throws {
                ...
                switch data[0] {
                case 0xE9, 0xE0, 0xC0, 0xA0, 0x49, 0x09, 0xC9, 0x29, 0x69, 0xA9, 0xA2:
                    self = .immediate(data[1])
                case 0x84, 0xE5, 0xE4, 0xC4, 0xA4, 0x45, 0x05, 0xC5, 0x25, 0x86, 0x46, 0xE6, 0x65, 0x26, 0xA5, 0x85, 0x24, 0xA6, 0x66:
                    self = .zeroPage(data[1])
                case 0x94, 0xF5, 0xB4, 0x55, 0x15, 0xD5, 0x35, 0x56, 0xF6, 0x75, 0x36, 0xB5, 0x95, 0x76:
                    self = .zeroPageX(data[1])
                case 0x96, 0xB6:
                    self = .zeroPageY(data[1])
                case 0x8C, 0xED, 0xEC, 0xCC, 0xAC, 0x4D, 0x0D, 0xCD, 0x2D, 0x8E, 0x4E, 0xEE, 0x6D, 0x2E, 0xAD, 0x8D, 0x20, 0x2C, 0xAE, 0x4C, 0x6E:
                    let address = (UInt16(data[2]) << 8) + UInt16(data[1])
                    self = .absolute(address)
                case 0xFD, 0xBC, 0x5D, 0x1D, 0xDD, 0x3D, 0x5E, 0xFE, 0x7D, 0x3E, 0xBD, 0x9D, 0x7E:
                    let address = (UInt16(data[2]) << 8) + UInt16(data[1])
                    self = .absoluteX(address)
                case 0xF9, 0x59, 0x19, 0xD9, 0x39, 0x79, 0xB9, 0x99, 0xBE:
                    let address = (UInt16(data[2]) << 8) + UInt16(data[1])
                    self = .absoluteY(address)
                case 0x6C:
                    let address = (UInt16(data[2]) << 8) + UInt16(data[1])
                    self = .indirect(address)
                case 0xE1, 0x41, 0x01, 0xC1, 0x21, 0x61, 0xA1, 0x81:
                    self = .indexedIndirect(data[1])
                case 0xF1, 0x51, 0x11, 0xD1, 0x31, 0x71, 0xB1, 0x91:
                    self = .indirectIndexed(data[1])
                case 0xB8, 0xD8, 0xF8, 0x60, 0x18, 0x38, 0xEA, 0x40, 0x00, 0x78, 0x9A, 0xBA, 0x48, 0x68, 0x08, 0x28, 0xAA, 0x8A, 0xCA, 0xE8, 0xA8, 0x98, 0x88, 0xC8:
                    self = .implied
                case 0x30, 0x50, 0xD0, 0x90, 0xB0, 0x70, 0xF0, 0x10:
